...
describe('commitMessageHandler', () => {
  ...
  describe('promptForCommitMessage', () => {
    ...
    it('should commit immediately in ai_automatic mode', async () => {
      ...
      await promptForCommitMessage('/test/repo', 'ai_automatic', mockCallback);
    ...
    it('should use fallback message when AI fails in automatic mode', async () => {
      const { generateCommitMessage } = await import('../../../src/utils/aiMerge.js');
      vi.mocked(generateCommitMessage).mockResolvedValue(null);
      ...
      expect(mockCallback).toHaveBeenCalledWith('chore: update files');
    });

    it('should return a visible error when diff collection fails in AI mode', async () => {
      const { getComprehensiveDiff } = await import('../../../src/utils/aiMerge.js');
      vi.mocked(getComprehensiveDiff).mockImplementationOnce(() => {
        throw new Error('failed to collect git diff');
      });
      ...
      expect(result.message).toContain('Unable to inspect changes');
      expect(mockCallback).not.toHaveBeenCalled();
    });
