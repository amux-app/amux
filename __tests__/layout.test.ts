...
import { calculateOptimalLayout, DEFAULT_LAYOUT_CONFIG, resolveControlPaneId } from '../src/utils/layoutManager.js';

describe('layout calculation', () => {
  describe('control pane recovery', () => {
    it('recovers when paneExists reports a missing control pane without throwing', async () => {
      const tmux = {
        paneExists: async () => false,
        getPanePositionsSync: () => [
          { paneId: '%sidebar', left: 0, width: 40 },
          { paneId: '%content', left: 40, width: 120 },
        ],
      };

      await expect(resolveControlPaneId(tmux, '%stale')).resolves.toBe('%sidebar');
    });

    it('keeps the configured control pane when it still exists', async () => {
      const tmux = {
        paneExists: async () => true,
        getPanePositionsSync: () => [],
      };

      await expect(resolveControlPaneId(tmux, '%control')).resolves.toBe('%control');
    });
  });
