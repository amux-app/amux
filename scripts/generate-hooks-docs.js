 ...
 * comprehensive documentation that gets embedded in the muxbase binary.
 */
...
const agentsMd = `# muxbase Hooks System - Agent Reference

**Auto-generated documentation for AI agents**

This document contains everything an AI agent needs to create, modify, and understand muxbase hooks. It is automatically generated from the muxbase source code and embedded in the binary.

## What You're Working On

You are editing hooks for **muxbase**, a tmux pane manager that creates AI-powered development workflows. Each pane runs in its own git worktree with an AI agent (Claude Code or opencode).

## Your Goal

Create executable bash scripts in \`.muxbase-hooks/\` that run automatically at key lifecycle events.

## Quick Start

1. **Create a hook file**: \`touch .muxbase-hooks/worktree_created\`
2. **Make it executable**: \`chmod +x .muxbase-hooks/worktree_created\`
3. **Add shebang**: Start with \`#!/bin/bash\`
4. **Use environment variables**: Access \`$MUXBASE_ROOT\`, \`$MUXBASE_WORKTREE_PATH\`, etc.
5. **Test it**: Set env vars manually and run the script

## Hook Execution Model

- **Non-blocking**: Hooks run in background (detached processes)
- **Silent failures**: Hook errors are logged but don't stop muxbase
- **Environment-based**: All context passed via environment variables
- **Project-local**: Hooks in \`.muxbase-hooks/\` are gitignored by default and should not be committed
- **Priority resolution**: \`.muxbase-hooks/\` → \`.muxbase/hooks/\` → \`~/.muxbase/hooks/\`
...
### Always Available
\`\`\`bash
MUXBASE_ROOT="/path/to/project"           # Project root directory
MUXBASE_METADATA_DIR="/path/to/project/.muxbase" # Project metadata directory
MUXBASE_HOOKS_DIR="/path/to/project/.muxbase-hooks" # Active project hooks directory
\`\`\`

### Pane Context (most hooks)
\`\`\`bash
MUXBASE_PANE_ID="muxbase-1234567890"         # muxbase pane identifier
MUXBASE_SLUG="fix-auth-bug"               # Branch/worktree name
MUXBASE_PROMPT="Fix authentication bug"   # User's prompt
MUXBASE_AGENT="claude"                    # Agent type (claude|opencode)
MUXBASE_TMUX_PANE_ID="%38"                # tmux pane ID
\`\`\`

\`MUXBASE_PROMPT\` contains user-provided task text. Treat it as sensitive local context and do not send it to third-party services from hooks unless that is intentional.

### Worktree Context
\`\`\`bash
MUXBASE_WORKTREE_PATH="/path/.muxbase/worktrees/fix-auth-bug"
MUXBASE_BRANCH="fix-auth-bug"             # Same as slug
\`\`\`

### Merge Context
\`\`\`bash
MUXBASE_TARGET_BRANCH="main"              # Branch being merged into
\`\`\`
...
\`\`\`bash
#!/bin/bash
# .muxbase-hooks/worktree_created
...
for file in .env.development .npmrc .yarnrc; do
  if [ -f "$MUXBASE_ROOT/$file" ]; then
    cp "$MUXBASE_ROOT/$file" "$MUXBASE_WORKTREE_PATH/$file"
  fi
done
\`\`\`
...
ISSUE=$(echo "$MUXBASE_PROMPT" | grep -oP '#\\K\\d+' | head -1)
if [ -n "$ISSUE" ] && command -v gh &> /dev/null; then
  gh issue close "$ISSUE" \\
    -c "Resolved in $MUXBASE_SLUG, merged to $MUXBASE_TARGET_BRANCH" \\
    2>/dev/null || true
fi
\`\`\`
...
3. **Make executable**: \`chmod +x .muxbase-hooks/hook_name\`
4. **Background long operations**: Append \`&\` to avoid blocking
5. **Check for required tools**: \`command -v tool &> /dev/null\`
6. **Log for debugging**: \`echo "[Hook] message" >> "$MUXBASE_METADATA_DIR/hooks.log"\`
...
export MUXBASE_PANE_ID="test-pane"
export MUXBASE_SLUG="test-branch"
export MUXBASE_WORKTREE_PATH="$(pwd)"
export MUXBASE_AGENT="claude"
export MUXBASE_PROMPT="Test prompt"
...
bash -n ./.muxbase-hooks/worktree_created
\`\`\`

### Shellcheck (if available)
\`\`\`bash
shellcheck ./.muxbase-hooks/worktree_created
\`\`\`
...
❌ **Blocking operations**: \`sleep 60\` (blocks muxbase)
✅ **Background long tasks**: \`slow_operation &\`

❌ **Hardcoded paths**: \`/Users/me/project\`
✅ **Use variables**: \`"$MUXBASE_ROOT"\`
...
❌ **Noisy output**: Clutters muxbase logs
...
1. **Check if file exists**: \`ls -la .muxbase-hooks/\`
2. **Check permissions**: Should show \`x\` in \`rwxr-xr-x\`
3. **Check syntax**: \`bash -n .muxbase-hooks/hook_name\`
4. **Test manually**: Set env vars and run
5. **Check logs**: muxbase logs to stderr with \`[Hooks]\` prefix
...
- **Claude-specific tips**: See \`CLAUDE.md\` in \`.muxbase-hooks/\`
- **Examples**: Check \`.muxbase-hooks/examples/\` directory
- **muxbase API**: See \`API.md\` for REST endpoints

---

*This documentation was auto-generated from muxbase source code.*
