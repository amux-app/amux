...
- **Impact:** High — hardens uncontrolled-input handling, marketplace Git invocation, hook configuration mutation, generated TypeScript encoding, and Electron test cleanup commands. No public API, IPC contract, dependency, or persisted-data schema changed.
- **What:** Fixed all 14 reported CodeQL sites: five polynomial/inefficient regular expressions, two second-order Git command-injection flows, two environment-derived shell commands, two prototype-polluting array assignments, incomplete generated-source escaping, and improper inline-script fixture sanitization. Duplicate slug/filename findings now share one implementation.
- **Why:** The alerts had five common causes: chained regular-expression replacements on uncontrolled names, backtracking searches over uncontrolled prompts/content, missing Git option termination, string-built shell commands, and mutation/encoding primitives that relied on context-sensitive syntax. Several sites were behaviorally safe in their current call paths, but their primitives were fragile and produced high-confidence security findings.
- **How:** Added a bounded one-pass ASCII name normalizer and reused it for worktree slugs, agent suffixes, and prompt filenames; replaced prompt and internal-host regex searches with linear scanners; encoded generated TypeScript through `JSON.stringify`; added `--` before Git clone operands; replaced dynamic array-index assignment with `splice` and special-key-safe `defineProperty`; passed tmux arguments through `execFileSync`; and escaped inline-script fixture delimiters after JSON serialization. Added focused hostile-length, metacharacter round-trip, Git option-boundary, and `__proto__` regressions.
- **Risk/compatibility:** Slug, filename, hook-scaffold detection, generated documentation, and test-cleanup behavior remain compatible with the covered prior semantics. Internal-host scanning is now intentionally limited to valid ASCII/punycode hostname characters. Marketplace hook event maps safely support own properties named `__proto__` without touching `Object.prototype`. Hosted CodeQL closure remains dependent on the next GitHub scan because the CodeQL CLI is not installed locally.
- **Validation:** The RED run failed at the new normalizer, prompt detector, source-literal encoder, and Git option-boundary assertions before implementation. The focused core security/regression set then passed 132/132. `pnpm test` passed 129 files with 1 skipped, 1,093 tests with 2 skipped. The added long-host scanner regression and its suite passed 15/15. `pnpm run verify:static` passed twice (brand guard, internal-reference gate, version alignment, TypeScript, zero-warning ESLint, and Knip). Desktop TypeScript checks passed, the Margin Lab parser suite passed 4/4, `git diff --check` passed, and the core build completed through desktop pre-typecheck.
