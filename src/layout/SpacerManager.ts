...
function resolveSpacerScriptPath(): string {
  const candidates = [
    join(__dirname, '..', 'panes', 'spacer-pane.js'),
    join(__dirname, '..', '..', 'dist', 'panes', 'spacer-pane.js'),
    join(process.cwd(), 'dist', 'panes', 'spacer-pane.js'),
  ];
...
export class SpacerManager {
  ...
  createSpacerPane(lastContentPaneId: string): string {
    ...
    const scriptPath = resolveSpacerScriptPath();

    const newPaneId = this.tmuxService.splitPaneSync({
      command: `node ${shQuote(scriptPath)}`,
    });
