...
/**
 * Pause/resume surface of the single config watcher owned by the host app.
 */
export interface ConfigWatcherControl {
  pause(): void;
  resume(): void;
}
...
export class StateManager extends EventEmitter {
  ...
  /**
   * The host app owns the config watcher and pushes file changes in via
   * updatePanes(); registering it here only exposes pause/resume to actions.
   */
  setConfigWatcher(control: ConfigWatcherControl | null): void {
    this.configWatcher = control;
  }
  ...
  private notifyListeners(): void {
    ...
    this.updateCallbacks.forEach(callback => {
      try {
        ...
        this.logService.error('Error in state listener', 'StateManager', undefined, err);
      }
    });
