 ...
 */
export function atomicWriteJsonSync(filePath: string, data: unknown, pretty: boolean = true): void {
 ...
 * @param pretty - Whether to pretty-print the JSON (default: true)
 */
export async function atomicWriteJson(filePath: string, data: unknown, pretty: boolean = true): Promise<void> {
  const content = pretty ? JSON.stringify(data, null, 2) : JSON.stringify(data);
