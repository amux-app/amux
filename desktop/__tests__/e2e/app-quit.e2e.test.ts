...
describe.runIf(process.env.MUXBASE_E2E === '1')('Application quit E2E', () => {
  ...
  beforeAll(async () => {
    ...
    projectRoot = mkdtempSync(join(tmpdir(), 'muxbase-quit-e2e-'));
    ...
    writeFileSync(fakeTmux, [
      '#!/bin/sh',
      'if [ ! -e "$MUXBASE_E2E_BLOCK_TMUX_FILE" ]; then',
      '  exec "$MUXBASE_E2E_REAL_TMUX" "$@"',
      'fi',
      'printf \'%s\\n\' "$$" >> "$MUXBASE_E2E_FAKE_TMUX_PID_FILE"',
      'exec /bin/cat "$MUXBASE_E2E_FAKE_TMUX_FIFO"',
      '',
    ].join('\n'));
    ...
    app = await electron.launch({
      ...
      env: {
        ...process.env,
        MUXBASE_E2E_BLOCK_TMUX_FILE: blockTmuxFile,
        MUXBASE_E2E_FAKE_TMUX_FIFO: blockingFifo,
        MUXBASE_E2E_FAKE_TMUX_PID_FILE: fakeTmuxPidFile,
        MUXBASE_E2E_REAL_TMUX: realTmuxPath,
  ...
  afterAll(async () => {
    ...
    if (realTmuxPath && projectRoot) {
      try {
        execFileSync(realTmuxPath, ['kill-session', '-t', `=muxbase-${basename(projectRoot)}`], {
          stdio: 'ignore',
        });
      } catch {
  ...
  it('force-exits when resource cleanup exceeds the global quit deadline', async () => {
    ...
    const accepted = await page.evaluate(async (channel) => {
      return (window as unknown as E2EWindow).muxbase.invoke<boolean>(channel);
    }, IPC.APP_QUIT);
