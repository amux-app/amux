...
const ACTIVE_ROOT = mkdtempSync(path.join(tmpdir(), 'muxbase-active-'));
const SECONDARY_ROOT = mkdtempSync(path.join(tmpdir(), 'muxbase-secondary-'));
const WORKTREE_ROOT = mkdtempSync(path.join(tmpdir(), 'muxbase-worktree-'));
...
describe('renderer project-root authorization', () => {
  ...
  it('rejects an arbitrary existing directory', async () => {
    const arbitrary = mkdtempSync(path.join(tmpdir(), 'muxbase-arbitrary-'));
  ...
  it('accepts a symlink alias only when its canonical target is authorized', async () => {
    const parent = mkdtempSync(path.join(tmpdir(), 'muxbase-alias-'));
  ...
  it('rejects a removed pane root and nested Kanban target root', async () => {
    const removed = mkdtempSync(path.join(tmpdir(), 'muxbase-removed-'));
    const nested = mkdtempSync(path.join(tmpdir(), 'muxbase-kanban-target-'));
