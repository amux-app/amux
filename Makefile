...
install: ## Install Amux.app from source (macOS only)
...
dev-worktree: core-build ## Run dev from a worktree on its own port + user-data dir (defaults to 5374; override AUMX_RENDERER_PORT for multiple worktrees)
	@case "$$(pwd)" in *.amux/worktrees/*|*.aumx/worktrees/*) ;; *) echo "✗ dev-worktree must run from inside .amux/worktrees/<name>"; exit 1 ;; esac
	@WT_ROOT=$$(pwd); \
	 PORT="$${AUMX_RENDERER_PORT:-5374}"; \
	 USER_DATA="$$WT_ROOT/.amux-dev/userdata"; \
	 mkdir -p "$$USER_DATA"; \
	 echo "→ worktree=$$(basename $$WT_ROOT) port=$$PORT userdata=$$USER_DATA"; \
	 cd $(DESKTOP_DIR) && AUMX_RENDERER_PORT="$$PORT" AUMX_USER_DATA_DIR="$$USER_DATA" pnpm exec electron-vite dev

core-build: ## Build aumx core (required for aumx/core ESM exports)
	pnpm -w --filter aumx build
...
test-e2e: core-build build ## Run all E2E tests hidden (set E2E_HEADED=1 to show Electron)
	cd $(DESKTOP_DIR) && AUMX_E2E_HEADED=$(E2E_HEADED) pnpm run test:e2e

test-e2e-stable: core-build build ## Run deterministic shell/UI E2E tests used by the release gate
	cd $(DESKTOP_DIR) && AUMX_E2E_HEADED=$(E2E_HEADED) pnpm run test:e2e:stable

test-e2e-live: core-build build ## Run live-agent E2E tests that depend on installed agent CLIs
	cd $(DESKTOP_DIR) && AUMX_E2E_HEADED=$(E2E_HEADED) pnpm run test:e2e:live

test-e2e-kanban: core-build build ## Run kanban board E2E test only
	cd $(DESKTOP_DIR) && AUMX_E2E=1 AUMX_E2E_HEADED=$(E2E_HEADED) AUMX_E2E_ALLOW_STORE_COERCE=1 pnpm exec vitest run --config vitest.config.ts --no-file-parallelism __tests__/e2e/kanban-board.e2e.test.ts

test-e2e-pane: core-build build ## Run multi-pane agent E2E test only
	cd $(DESKTOP_DIR) && AUMX_E2E=1 AUMX_E2E_HEADED=$(E2E_HEADED) AUMX_E2E_ALLOW_STORE_COERCE=1 pnpm exec vitest run --config vitest.config.ts --no-file-parallelism __tests__/e2e/multi-pane-agent.e2e.test.ts

test-e2e-multi-agent: core-build build ## Run multi-agent (Claude + OpenCode) conversation E2E test only
	cd $(DESKTOP_DIR) && AUMX_E2E=1 AUMX_E2E_HEADED=$(E2E_HEADED) AUMX_E2E_ALLOW_STORE_COERCE=1 pnpm exec vitest run --config vitest.config.ts --no-file-parallelism __tests__/e2e/multi-agent-conversation.e2e.test.ts

test-e2e-live-agent-terminal: core-build build ## Run live Claude/OpenCode terminal scroll fidelity E2E (starts hai proxy)
	cd $(DESKTOP_DIR) && AUMX_E2E=1 AUMX_E2E_HEADED=$(E2E_HEADED) AUMX_E2E_LIVE_AGENTS=1 AUMX_E2E_ALLOW_STORE_COERCE=1 pnpm exec vitest run --config vitest.config.ts --no-file-parallelism --testTimeout=1200000 __tests__/e2e/live-agent-terminal-scroll.e2e.test.ts
...
test-e2e-multi-agent-slow: core-build build ## Run multi-agent E2E in slow visual mode (pauses, on-screen overlays, UI-driven creation, holds window open at end)
	cd $(DESKTOP_DIR) && AUMX_E2E=1 AUMX_E2E_HEADED=1 AUMX_E2E_ALLOW_STORE_COERCE=1 AUMX_E2E_SLOW=1 pnpm exec vitest run --config vitest.config.ts --no-file-parallelism --testTimeout=1200000 __tests__/e2e/multi-agent-conversation.e2e.test.ts

test-e2e-conflict: core-build build ## Run conflict resolution E2E test only
	cd $(DESKTOP_DIR) && AUMX_E2E=1 AUMX_E2E_HEADED=$(E2E_HEADED) pnpm exec vitest run --config vitest.config.ts --no-file-parallelism __tests__/e2e/conflict-resolution.e2e.test.ts

test-e2e-conflict-slow: core-build build ## Run conflict resolution E2E slowly (visual demo)
	cd $(DESKTOP_DIR) && AUMX_E2E=1 AUMX_E2E_HEADED=1 AUMX_E2E_SLOW=1 pnpm exec vitest run --config vitest.config.ts --no-file-parallelism __tests__/e2e/conflict-resolution.e2e.test.ts

demo-record: core-build build ## Record demo footage (hero.webm + full.webm) via the demo E2E test
	cd $(DESKTOP_DIR) && AUMX_DEMO_VIDEO=1 AUMX_E2E=1 AUMX_E2E_HEADED=$(E2E_HEADED) pnpm exec vitest run --config vitest.config.ts --no-file-parallelism __tests__/e2e/demo-video.e2e.test.ts
...
open: pack ## Package and launch the app
	@open "$$(ls -d $(DESKTOP_DIR)/release/mac*/Amux.app | head -1)"

all: install ## Install Amux.app from source
